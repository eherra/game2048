...
Game 2048 where you need to move squares on board and try to sum same value numbers with each other. The game is lost when there're no moves left (squares are not summing with each other (up, down, left or right)), and won when one of the square's value is 2048.
